...
> ⏸ **Track-B — REMAINING (needs an operator decision/secret or infra, out of scope for the in-repo batch):** 🟠 **OPS-DB-1** (offsite + encrypted backups), 🟠 **OPS-OBS-1** (alert delivery / real alertmanager receiver), 🟠 **OPS-CI-1** (CD pipeline), ⚪ OPS-DEP-1/2 (Caddy headers / rootfs), ⚪ OPS-CI-3 (image vuln scan), ⚪ OPS-OBS-4 (metrics-token scrape coupling), ⚪ OPS-OBS-5 (write-path perf scenario). Plus 🟡 **OPS-TEST-2** (open-ended broad unit-coverage expansion). Each, when pursued, gets its own spec. Also noted: the full `npm run verify` **e2e suite is environmentally flaky under load** (unrelated suites fail intermittently, pass in isolation — a Testcontainers-contention artifact) — a candidate ops follow-up (e.g. resource limits / run tuning).
