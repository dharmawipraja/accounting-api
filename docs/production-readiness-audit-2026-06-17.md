...
> ⏸ **§4 REMAINING — operator activation + one open-ended item (no in-repo code left):** the High items now ship as inert scaffolding that the operator **activates by supplying secrets/targets**: 🟠 OPS-DB-1 (set `BACKUP_AGE_RECIPIENT`/`BACKUP_S3_BUCKET`/etc. + add tools to the backup image), 🟠 OPS-OBS-1 (set a real webhook/Slack receiver), 🟠 OPS-CI-1 (push to GitHub + add `DEPLOY_SSH_*` secrets). Plus 🟡 **OPS-TEST-2** (open-ended broad unit-coverage expansion — its own spec; the Track-A coverage floor already guards regression). Minor follow-ups (non-blocking, from the whole-branch review): SHA-pin the CI Trivy action for consistency; branch-protection when CD activates. Also noted: the full `npm run verify` **e2e suite is environmentally flaky under load** (unrelated suites fail intermittently, pass in isolation — a Testcontainers-contention artifact; confirm any failure in isolation).
