...
- **SEC-2 — Idempotency-Key header unvalidated/unbounded** — `idempotency.interceptor.ts:40–44`, stored verbatim as PK (`IdempotencyKey.key String @id`, no length cap), no TTL/cleanup. Authenticated abuse → table/storage growth. **Fix:** validate (UUID or ≤128 chars, else 422) before `reserve()`; add a scheduled purge of completed keys. ✅ FIXED (branch fix/security-hardening) — invalid keys reject with **422** (`ValidationFailedError`), not 400.
- **SEC-3 — Login throttle is X-Forwarded-For spoofable** — `src/common/guards/user-throttler.guard.ts:16–24` + `main.ts` `trust proxy: 1`. Anonymous routes key on `ip:${req.ip}`; Caddy (`reverse_proxy api:3000`) doesn't strip/overwrite an inbound `X-Forwarded-For`, so a client-supplied XFF rotates the perceived IP and defeats the per-IP login limit (10/min). **Fix:** have Caddy set a trusted client header (`header_up X-Forwarded-For {remote_host}` or `trusted_proxies`) and overwrite inbound XFF; consider also keying login throttling by submitted email. ✅ FIXED app-side (branch fix/security-hardening): login now keyed by submitted email, not spoofable IP. Caddy trusted-proxy/XFF config remains deferred (infra item). ✅ Resolved — the app-side email-keyed login throttle was the real fix (already merged). The Caddy-side concern was based on an inaccurate premise: Caddy's reverse_proxy already ignores client-supplied X-Forwarded-For by DEFAULT (spoof protection), so the app already receives the real client IP — no Caddy directive needed. Documented in docs/runbooks/deploy.md (incl. trusted_proxies guidance for a future CDN/LB).
...
- **SEC-7 — Audit log append-only by convention only** — `src/audit/audit.service.ts:22–40` (create-only; read is ADMIN-gated, good). Table is writable by the app's DB role. **Fix:** revoke UPDATE/DELETE on `audit_log` from the app role (or a trigger / append-only sink); at minimum document the limitation. Deferred — DB role grant change is an infra item; no app-code fix in this branch. ✅ FIXED (branch fix/security-infra-hardening) — append-only enforced via a BEFORE UPDATE OR DELETE trigger (migration 20260617000001_audit_log_append_only); a REVOKE would no-op against the single owning 'accounting' role.
- **SEC-8 — `npm audit` gate scope** — `package.json` `audit:ci` (`--omit=dev --audit-level=high`) ignores dev deps and moderate advisories. **Fix:** consider `--audit-level=moderate` for prod deps + a non-blocking dev scan. (Also: CI has no remote yet, so the gate isn't running.) ✅ FIXED (branch fix/security-hardening) ✅ Workflow + gate already in place (.github/workflows/ci.yml: verify/audit/docker; audit:ci now --audit-level=moderate); activation is an operator step (add a git remote + push), documented in docs/runbooks/deploy.md.
...
>
> **Remaining §3 remnants — now closed in-repo (2026-06-18):**
> - ✅ Nit **#2 FIXED** — both `PaginationQueryDto` and `audit-query.dto.ts` now use `@Max(MAX_LIMIT)` (audit reconciled `500`→`200`); the cap is the single hoisted constant, so it can't drift again. `openapi.json` regenerated (audit `limit.maximum` 500→200).
> - ✅ Cross-repo `.data` unwrap — **documented as FE migration guidance** in `docs/api/frontend-guide.md` and `docs/api/frontend-agent-brief.md` (the latter was also stale — it still listed accounts/tax-codes as bare arrays; corrected). The actual code change lands in the sibling `accounting-client` repo.
>
> **§3 is fully resolved in this repo; remaining audit backlog is §4 (ops/DR) only.**
...
> **Status (updated 2026-06-18):** ✅ **Track-A (in-repo, decision-free) RESOLVED & merged to `main`** (fast-forward `886c89a`, branch `feat/ops-track-a`; spec/plan `docs/superpowers/{specs,plans}/2026-06-18-ops-track-a*`; whole-branch opus review clean). Closed: **OPS-CFG-1** (CORS_ORIGIN/ENABLE_SWAGGER/LOG_LEVEL now in `env.validation.ts`), **OPS-CFG-2** (fail-closed `parseCorsOrigins` trim/filter), **OPS-OBS-2** (Sentry `beforeSend` PII scrub), **OPS-OBS-3** (`LOG_LEVEL` pino wiring), **OPS-RES-1** (uncaughtException→exit(1) / unhandledRejection→log handlers), **OPS-CI-2** (ratcheted unit `coverageThreshold` 18/12/14/18), **OPS-DOC-1/2/3/4** (rate-limit policy, Redis prereq, concrete rollback, monitoring overlay), **OPS-TEST-1** (document-number + document-posting unit specs), **OPS-TEST-3** (journal-e2e `randomUUID` keys). No single-threaded behavior change. Unit grew to 114 tests.
> ✅ **Track-B (infra hardening + scaffolding) RESOLVED & merged to `main`** (fast-forward `686c199`, branch `feat/ops-track-b`; spec/plan `docs/superpowers/{specs,plans}/2026-06-18-ops-track-b*`; whole-branch opus review clean). No app code; all scaffolding merges **inert** (no behavior change until the operator sets env/secrets). Closed: **OPS-DEP-1** (Caddy security headers), **OPS-DEP-2** (read_only+cap_drop+tmpfs on api/migrate), **OPS-CI-3** (Trivy image scan in CI), **OPS-DB-1** (gated offsite + age-encrypted backup scaffolding), **OPS-OBS-1** (ready-to-activate alertmanager webhook receiver), **OPS-CI-1** (CD workflow: GHCR publish + secret-gated SSH deploy, third-party actions SHA-pinned), **OPS-OBS-4** (metrics-token coupling doc), **OPS-OBS-5** (opt-in k6 write scenario). **Bonus — fixed a real production-blocker** the read-only smoke exposed: `Dockerfile` CMD and `package.json` `start:prod` both pointed at `dist/main.js`, but the build emits `dist/src/main.js` (outDir `./dist` + nest-cli `sourceRoot src`) → the prod image was dead-on-arrival; both call-sites corrected (`955def9`, `9c78767`).
> ✅ **Remnants RESOLVED & merged to `main` 2026-06-19** (fast-forward `a07ecc4`, branch `feat/audit-remnants`; spec/plan `docs/superpowers/{specs,plans}/2026-06-19-audit-remnants*`; whole-branch opus review clean). An independent code-verification pass first confirmed **every Critical/High/Medium finding genuinely fixed in source**. Then the last actionable in-repo Low/Medium items: **OPS-RES-2** (per-request timeout interceptor → 408, `REQUEST_TIMEOUT_MS`), **OPS-CFG-3** (every optional `EnvVars` now in `.env.example`), **OPS-TEST-2** (focused pure-logic unit specs — tax engine, `assertBalanced` extracted to a pure helper, BalancesService signing) + the unit coverage floor raised 18/12/14/18 → **22/18/18/22** (ties off OPS-CI-2). No app behavior change except a slow request now 408. Unit 135 tests / 31 suites.
> ⏸ **§4 REMAINING — operator activation + deferred/backlog only (NO actionable in-repo code left):** the scaffolds activate when the operator supplies secrets/targets — 🟠 OPS-DB-1 (`BACKUP_AGE_RECIPIENT`/`BACKUP_S3_BUCKET` + backup-image tools), 🟠 OPS-OBS-1 (real webhook/Slack receiver), 🟠 OPS-CI-1 (push to GitHub + `DEPLOY_SSH_*` secrets). Deferred-by-design: 🟡 **OPS-TEST-2 deepening** (year-end close + broader engine coverage stay e2e — deliberate, not unit-mocked); ⚪ OPS-DB-2 (already-applied trigram migration — not editable); OPS-RES-2 follow-up (env-drive `server.requestTimeout` so `REQUEST_TIMEOUT_MS` > 30s doesn't socket-cut before the 408 — documented caveat shipped); SHA-pin the CI Trivy action; branch-protection when CD activates. Not-a-bug: OPS-DB-3, NEW-2. Also noted: the full `npm run verify` **e2e suite is environmentally flaky under load** (unrelated suites fail intermittently, pass in isolation — confirm any failure in isolation). **The entire audit's in-repo code backlog (§1–§4) is now closed.**
