...
| **Alert delivery** (OPS-OBS-1, High) | Replace the placeholder receiver in `monitoring/alertmanager.yml` with a real Slack/email/webhook target. The alert *rules* exist and fire; right now they reach no one. | `monitoring/alertmanager.yml`, [deploy.md](./deploy.md) |
